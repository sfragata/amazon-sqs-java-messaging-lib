...
public class SQSMessageTest {
    ...
    public void setup() {
        ...
        this.mockSQSSession = mock(SQSSession.class);
    }
    ...
    public void testProperty()
        ...
        when(this.mockSQSSession.createMessage()).thenReturn(new SQSMessage());
        Message message = this.mockSQSSession.createMessage();
        ...
        Set<String> propertyNamesSet =
            new HashSet<String>(
                Arrays.asList("myTrueBoolean", "myFalseBoolean", "myInteger", "myDouble", "myFloat", "myLong",
                    "myShort", "myByteProperty", "myNumber", "myString"));
     ...
     */
    @Test
    public void testCheckPropertyWritePermissions()
        throws JMSException {
     ...
     */
    @Test
    public void testSQSMessageAttributeToProperty()
        throws JMSException {
        ...
        messageAttributes.put(this.myTrueBoolean,
            MessageAttributeValue.builder().dataType(SQSMessagingClientConstants.BOOLEAN).stringValue("1").build());

        messageAttributes.put(this.myFalseBoolean,
            MessageAttributeValue.builder().dataType(SQSMessagingClientConstants.BOOLEAN).stringValue("0").build());

        messageAttributes.put(this.myInteger,
            MessageAttributeValue.builder().dataType(SQSMessagingClientConstants.INT).stringValue("100").build());

        messageAttributes.put(this.myDouble,
            MessageAttributeValue.builder().dataType(SQSMessagingClientConstants.DOUBLE).stringValue("2.1768").build());

        messageAttributes.put(this.myFloat,
            MessageAttributeValue.builder().dataType(SQSMessagingClientConstants.FLOAT).stringValue("3.1457").build());

        messageAttributes.put(this.myLong, MessageAttributeValue.builder().dataType(SQSMessagingClientConstants.LONG)
            .stringValue("1290772974281").build());

        messageAttributes.put(this.myShort,
            MessageAttributeValue.builder().dataType(SQSMessagingClientConstants.SHORT).stringValue("123").build());

        messageAttributes.put(this.myByte,
            MessageAttributeValue.builder().dataType(SQSMessagingClientConstants.BYTE).stringValue("1").build());

        messageAttributes.put(this.myString, MessageAttributeValue.builder()
            .dataType(SQSMessagingClientConstants.STRING).stringValue("StringValue").build());

        messageAttributes.put(this.myCustomString, MessageAttributeValue.builder()
            .dataType(SQSMessagingClientConstants.NUMBER + ".custom").stringValue("['one', 'two']").build());

        messageAttributes.put(this.myNumber,
            MessageAttributeValue.builder().dataType(SQSMessagingClientConstants.NUMBER).stringValue("500").build());

        software.amazon.awssdk.services.sqs.model.Message sqsMessage =
            software.amazon.awssdk.services.sqs.model.Message.builder().messageAttributes(messageAttributes)
                .attributesWithStrings(systemAttributes).messageId("messageId").receiptHandle("ReceiptHandle").build();

        SQSMessage message = new SQSMessage(ack, "QueueUrl", sqsMessage);

        Assert.assertTrue(message.propertyExists(this.myTrueBoolean));
        Assert.assertEquals(message.getObjectProperty(this.myTrueBoolean), true);
        Assert.assertEquals(message.getBooleanProperty(this.myTrueBoolean), true);

        Assert.assertTrue(message.propertyExists(this.myFalseBoolean));
        Assert.assertEquals(message.getObjectProperty(this.myFalseBoolean), false);
        Assert.assertEquals(message.getBooleanProperty(this.myFalseBoolean), false);

        Assert.assertTrue(message.propertyExists(this.myInteger));
        Assert.assertEquals(message.getObjectProperty(this.myInteger), 100);
        Assert.assertEquals(message.getIntProperty(this.myInteger), 100);

        Assert.assertTrue(message.propertyExists(this.myDouble));
        Assert.assertEquals(message.getObjectProperty(this.myDouble), 2.1768);
        Assert.assertEquals(message.getDoubleProperty(this.myDouble), 2.1768);

        Assert.assertTrue(message.propertyExists(this.myFloat));
        Assert.assertEquals(message.getObjectProperty(this.myFloat), 3.1457f);
        Assert.assertEquals(message.getFloatProperty(this.myFloat), 3.1457f);

        Assert.assertTrue(message.propertyExists(this.myLong));
        Assert.assertEquals(message.getObjectProperty(this.myLong), 1290772974281L);
        Assert.assertEquals(message.getLongProperty(this.myLong), 1290772974281L);

        Assert.assertTrue(message.propertyExists(this.myShort));
        Assert.assertEquals(message.getObjectProperty(this.myShort), (short) 123);
        Assert.assertEquals(message.getShortProperty(this.myShort), (short) 123);

        Assert.assertTrue(message.propertyExists(this.myByte));
        Assert.assertEquals(message.getObjectProperty(this.myByte), (byte) 1);
        Assert.assertEquals(message.getByteProperty(this.myByte), (byte) 1);

        Assert.assertTrue(message.propertyExists(this.myString));
        Assert.assertEquals(message.getObjectProperty(this.myString), "StringValue");
        Assert.assertEquals(message.getStringProperty(this.myString), "StringValue");

        Assert.assertTrue(message.propertyExists(this.myCustomString));
        Assert.assertEquals(message.getObjectProperty(this.myCustomString), "['one', 'two']");
        Assert.assertEquals(message.getStringProperty(this.myCustomString), "['one', 'two']");

        Assert.assertTrue(message.propertyExists(this.myNumber));
        Assert.assertEquals(message.getObjectProperty(this.myNumber), "500");
        Assert.assertEquals(message.getStringProperty(this.myNumber), "500");
        Assert.assertEquals(message.getIntProperty(this.myNumber), 500);
        Assert.assertEquals(message.getShortProperty(this.myNumber), (short) 500);
        Assert.assertEquals(message.getLongProperty(this.myNumber), 500l);
        Assert.assertEquals(message.getFloatProperty(this.myNumber), 500f);
        Assert.assertEquals(message.getDoubleProperty(this.myNumber), 500d);

        // Validate property names
        Set<String> propertyNamesSet =
            new HashSet<String>(
                Arrays.asList(this.myTrueBoolean, this.myFalseBoolean, this.myInteger, this.myDouble, this.myFloat,
                    this.myLong, this.myShort, this.myByte, this.myString, this.myCustomString, this.myNumber,
                    JMSX_DELIVERY_COUNT));
